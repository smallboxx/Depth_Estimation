...
class depth_mpl(nn.Module):
    def __init__(self,in_channels, patch_size=10, embedding_dim=128, num_heads=4, use_class_token=True):
        super().__init__()
        self.patch_transformer = PatchTransformerEncoder(
            in_channels, patch_size, embedding_dim, num_heads, use_class_token)
    ...
    def forward(self, x):
        fea = self.patch_transformer(x)
        out = self.mlp_classifier(fea[0])
        return out
