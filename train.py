...
def main():
    ...
    if args.finetune_model is not None:
        model_weight = torch.load(args.finetune_model)
        # if 'module' in next(iter(model_weight.items()))[0]:
        #     model_weight = OrderedDict((k[7:], v) for k, v in model_weight.items())
        model_weight = OrderedDict((f'module.{k}', v) for k, v in model_weight.items())
    ...
    Domain_classifier_loss = nn.MSELoss()
    ...
    for epoch in range(start_ep, args.epochs + 1):
        ...
        loss_train = train(train_loader, model, criterion_d, Domain_classifier_loss, log_txt, optimizer=optimizer, scaler=scaler, 
        ...
        if epoch % args.val_freq == 0:
            model.infer(True)
            results_dict, loss_val = validate(val_loader, model, criterion_d, Domain_classifier_loss,
                                              device=device, epoch=epoch, args=args,
                                              log_dir=log_dir)
            if results_dict['abs_rel']<loss_abs_rel:
                loss_abs_rel = results_dict['abs_rel']
...
def train(train_loader, model, criterion_d, Domain_classifier_loss, log_txt, optimizer, scaler, device, epoch, args):    
    global global_step
    model.train()
    ...
    for batch_idx, batch in enumerate(train_loader):    
        ...
        with amp.autocast():
            ...
            domain_pred = preds['domain'].float()

            loss = 0
            # print(domain_pred, domain_gt)
            loss_w = Domain_classifier_loss(domain_gt, domain_pred)
            losses['DomainLoss'] = loss_w
            loss = loss + 0.4 * loss_w
            loss_d = criterion_d(preds['pred_d'], depth_gt) 
            losses['SilogLoss']  = loss_d
            loss = loss + 0.6 * loss_d

        scaler.scale(loss).backward()
        scaler.step(optimizer)
        scaler.update()

        depth_loss.update(loss.item(), input_RGB.size(0))
...
def validate(val_loader, model, criterion_d,Domain_classifier_loss, device, epoch, args, log_dir):
    depth_loss = logging.AverageMeter()
    model.eval()
    ...
    for batch_idx, batch in enumerate(val_loader):
        ...
        domain_pred = pred['domain'].float()
        print(domain_pred.squeeze(0),'\n',domain_gt.squeeze(0))
   
        loss_w = Domain_classifier_loss(domain_pred, domain_gt)

        loss = 0.6*loss_d + 0.4*loss_w
